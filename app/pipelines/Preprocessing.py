...
class FeatureEncoder(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X:pd.DataFrame):
        ...
        for col, cat in columns.items():
            ...
            ohe = OneHotEncoder(categories=[cat],
                                sparse_output = False,
        ...
        for col in X.columns:
            X[col] = X[col].apply(lambda x: 0.0 if type(x) is str and 'No' in x else x)   # change all substrings with 'No' to 0
            X[col] = X[col].apply(lambda x: 1.0 if type(x) is str and 'Yes' in x else x)  # change all substrings with 'Yes' to 1
